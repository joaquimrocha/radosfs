...
ub8 hash64( k, length, level) /* was: hash() */
...
/*
--------------------------------------------------------------------
Parameterized version of hash64; throws in an extra 8-byte value to 
hash in addition to the contiguous block of memory.
(AGULATI)
--------------------------------------------------------------------
*/
ub8 hashParam64( k, length, moreData, level) /* was: hash() */
register ub1 *k;        /* the key */
register ub8  length;   /* the length of the key */
ub8  moreData;			/* blah */ 
register ub8  level;    /* the previous hash, or an arbitrary value */
